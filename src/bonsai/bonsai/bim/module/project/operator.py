...
class LoadLinkedProject(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if storeys:
            ...
            unassigned_col = bpy.data.collections.get("(Unassigned)") or bpy.data.collections.new("(Unassigned)")
            if unassigned_col.name not in self.collection.children:
...
class XYZTool(bpy.types.Operator, PolylineOperator):
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            ...
            if snap_pt is None:
                return {"RUNNING_MODAL"}
            XYZDecorator.commit_point(snap_pt.copy())
...
class ZTool(bpy.types.Operator, PolylineOperator):
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            ...
            if snap_pt is None:
                return {"RUNNING_MODAL"}
            ZDecorator.commit_point(snap_pt.copy())
